...
PPM* readPPM(char *input)
{
	...
	ppm->content = (Pixel**) malloc(ppm->header.height * sizeof(Pixel*));//Init the first dimension of the matrix
	for(int y = 0; y < ppm->header.height; y++)
	{
		ppm->content[y] = (Pixel*) malloc(ppm->header.width * sizeof(Pixel));//Init the second dimension of the matrix
		for(int x = 0; x < ppm->header.width; x++)
		{
			...
			ppm->content[y][x] = *pixel;//Set the pixel
...
int writePPM(char *output, PPM* ppm)
{
	...
	for(int y = 0; y < ppm->header.height; y++)
	{
		for(int x = 0; x < ppm->header.width; x++)
		{
			fprintf(out, "%c%c%c", getRed(&ppm->content[y][x]), getGreen(&ppm->content[y][x]), getBlue(&ppm->content[y][x]));
			writed += sizeof(Pixel);
		}
	}
